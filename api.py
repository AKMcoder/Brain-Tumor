...
app = Flask(__name__)
UPLOAD_FOLDER = "E:/Misc/Major-Project/App/static/uploads"

os.environ["TF_FORCE_GPU_ALLOW_GROWTH"] = "true"

classificationModel = load_model("detection/classification_model.h5")

segmentationModel = model_from_json(open("segmentation/ResUNet-seg-model.json").read())
segmentationModel.load_weights(
    os.path.join(os.path.dirname("segmentation/ResUNet-seg-model.json"), "seg_model.h5")
)
...
def prediction(test_image, model, model_seg, fileName):
    ...
    im.save(fileName)
    shutil.move(
        "E:/Misc/Major-Project/App/" + fileName,
        "E:/Misc/Major-Project/App/static/output/" + fileName,
    )
    return result
